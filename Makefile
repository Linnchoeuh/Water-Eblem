...
$(NAME):$(OBJ)
	...
	$(ECHO) $(RED) \
		It may doesn't work because you didn't compile libs, \
		then try : make lib $(DEFLT))
...
lib: FORCE
	@for path in $(LIBSDIRECTORIES); do \
		...
		cd $$path && \
		make re && \
		a=$$(find -name "*.a" -printf '../../$(INCLUDE)/%f' | sed 's/\.[[:alpha:]]*$$//') && \
		rm -d -r -f $$a && \
		cp -r ./include ../../$(INCLUDE) && \
		mv ../../$(INCLUDE)/include $$a && \
		mv *.a ../../ && \
		cd ../../; \
	done
